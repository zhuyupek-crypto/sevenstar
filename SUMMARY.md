...
> - 本报告统一采用 POOL_BAK 37 只口径（与 run_sweep.py 一致）
> 详见下方"P0 修复说明"。
...
**审计范围澄清**（重要）：
- 上述 84/150（56%）和 80/147（54%）污染率均对应 **2022-2023 IS 区间**，因为大池两组实验的 JSON 仅含 IS 数据。
- 此前报告中的 **C+E OOS 2024 收益 91.24%** 和大池 OOS 2024 收益 88.78% **没有可审计的 OOS 原始交易文件**（实验未跑到 2024）。
- 因此 91.24% / 88.78% 的撤回理由是：底层代码存在 fail-open 缺陷 + 缺少可审计的 OOS 原始结果，**不是**"已精确证明 OOS 也有 54% 污染"。
- 54% 污染率只能用于证明 IS 结果无效，不能外推到 OOS。
